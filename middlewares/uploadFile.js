...
const fileUpload = (imageFile) => {
  ...
  // maksimal size file
  const byteLimit = 1000 * 1000
  const maxSizeMb = 2 * byteLimit
  const upload = multer({ storage: storage, fileFilter: fileFilter, limits: {fileSize: maxSizeMb } }).single(imageFile)
